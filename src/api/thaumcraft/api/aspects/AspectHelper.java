...
public class AspectHelper {
	...
	public static AspectList getEntityAspects(Entity entity) {
		...
		if (entity instanceof EntityPlayer) {
			...
			f1: for (ThaumcraftApi.EntityTags et : CommonInternals.scanEntities) {
				if (!et.entityName.equals(EntityList.getEntityString(entity)))
					continue;
				if (et.nbts == null || et.nbts.length == 0) {
					...
					for (EntityTagsNBT nbt : et.nbts) {
						if (tc.hasKey(nbt.name)) {
							if (!ThaumcraftApiHelper.getNBTDataFromId(tc, tc.getTagId(nbt.name), nbt.name)
									.equals(nbt.value))
								continue f1;
						} else {
							continue f1;
