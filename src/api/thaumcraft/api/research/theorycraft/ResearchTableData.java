...
public class ResearchTableData {
	...
	/*
	 * categoryTotals stores the amount of progress per research category. Each
	 * point = 1% of progress towards a full theory.
	 */
	...
	public NBTTagCompound serialize() {
		...
		if (lastDraw != null)
			nbt.setTag("lastDraw", serializeCardChoice(lastDraw));
	...
	public void drawCards(int draw, EntityPlayer pe) {
		...
		while (draw > 0) {
			if (!aidDrawn && !aidCards.isEmpty() && pe.getRNG().nextFloat() <= .25) {
				...
				if (card == null || card.getInspirationCost() > insp || isCategoryBlocked(card.getResearchCategory()))
					continue;
