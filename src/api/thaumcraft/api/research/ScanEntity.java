...
public class ScanEntity implements IScanThing {
	...
	/**
	 * false if the specific entity class should be used, or true if anything the
	 * inherits from that class is also allowed.
	 */
		...
		if (obj != null && ((!inheritedClasses && entityClass == obj.getClass())
			...
			if (NBTData != null && NBTData.length > 0) {
				...
				for (EntityTagsNBT nbt : NBTData) {
					if (!tc.hasKey(nbt.name) || !ThaumcraftApiHelper
							.getNBTDataFromId(tc, tc.getTagId(nbt.name), nbt.name).equals(nbt.value)) {
						return false;
					}
				}
			}
