...
public class ScanningManager {
	 ...
	 * Add things to scan
	 * 
	 * @example <i>ScanManager.addScannableThing(new ScanItem("HIPSTER",new
	 *          ItemStack(Items.apple,1,OreDictionary.WILDCARD_VALUE)));</i><br>
	 *          This will unlock the <b>HIPSTER</b> research if you scan any kind of
	 *          apple.
	 */
	 ...
	 * @param player
	 * @param object
	 *            this could in theory be anything, but vanilla tc scanning tools
	 *            only pass in Entity, BlockPos, Itemstack or null
	 */
	public static void scanTheThing(EntityPlayer player, Object object) {
		...
		if (object instanceof BlockPos
				&& player.getEntityWorld().getTileEntity((BlockPos) object) instanceof IInventory) {
	 ...
	 * @param player
	 * @param object
	 * @return true if the object can be scanned for research the player has not yet
	 *         discovered
	 */
	...
	public static ItemStack getItemFromParms(EntityPlayer player, Object obj) {
		...
		if (obj instanceof BlockPos) {
			...
			try {
				if (is == null)
					is = state.getBlock().getPickBlock(state, rayTrace(player), player.world, (BlockPos) obj, player);
			} catch (Exception e) {
			}
