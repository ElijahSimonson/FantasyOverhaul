...
public class RechargeHelper {
	...
	/**
	 * This method is called to recharge an item from the aura. Call it wherever you
	 * need to call it from, but only if you add some way to recharge such items. In
	 * general this should only be called every 5 ticks.
	 * 
	 * @param world
	 * @param is
	 * @param pos
	...
	public static float rechargeItem(World world, ItemStack is, BlockPos pos, EntityPlayer player, int amt) {
		if (is == null || !(is.getItem() instanceof IRechargable))
			return 0;
	...
	/**
	 * This method is called to recharge an item. Nothing is drained from the aura
	 * in this version - it is just recharged blindly.
	 * 
	 * @param is
	 * @param player
	 *            May be null
	...
	public static int getCharge(ItemStack is) {
		...
		if (is.hasTagCompound())
			return is.getTagCompound().getInteger(NBT_TAG);
		return 0;
	}
