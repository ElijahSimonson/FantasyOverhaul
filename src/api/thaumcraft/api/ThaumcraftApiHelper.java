...
public class ThaumcraftApiHelper {
	...
	public static final IAttribute CHAMPION_MOD = (new RangedAttribute((IAttribute) null, "tc.mobmod", -2D, -2D, 100D))
			.setDescription("Champion modifier").setShouldWatch(true);
	...
	public static boolean areItemStackTagsEqualForCrafting(ItemStack slotItem, ItemStack recipeItem) {
		...
		while (iterator.hasNext()) {
			...
			if (slotItem.getTagCompound().hasKey(s)) {
				if (!slotItem.getTagCompound().getTag(s).toString()
						.equals(recipeItem.getTagCompound().getTag(s).toString())) {
					return false;
				}
			} else {
				return false;
	...
	public static RayTraceResult rayTraceIgnoringSource(World world, Vec3d v1, Vec3d v2, boolean bool1, boolean bool2,
			boolean bool3) {
				...
				while (k1-- >= 0) {
					...
					if (!bool2 || block1.getCollisionBoundingBox(world, new BlockPos(l, i1, j1)) != null) {
						if (block1.getBlock().canCollideCheck(block1, bool1)) {
							RayTraceResult rayTraceResult1 = block1.collisionRayTrace(world, new BlockPos(l, i1, j1),
									v1, v2);
							...
							rayTraceResult2 = new RayTraceResult(RayTraceResult.Type.MISS, v1, enumfacing,
									new BlockPos(l, i1, j1));
						}
					}
				}
	...
	/**
	 * Create a crystal itemstack from a sent aspect. Sending a null will result in
	 * a balanced shard (one of each primal).
	 * 
	 * @param aspect
	 * @return
	 */
