...
public class GolemHelper {
	 ...
	 * @param seal
	 * @param stack
	 *            the stack requested. Can accept wildcard values.
	 */
	...
	/**
	 * This method is used to get a single blockpos from within a designated seal
	 * area. This method is best used if you want to increment through the blocks in
	 * the area.
	 * 
	 * @param seal
	 * @param count
	 *            a value used to derive a specific pos
	 * @return
	 */
	public static BlockPos getPosInArea(ISealEntity seal, int count) {
		...
		BlockPos p = seal.getSealPos().pos.add(x - (seal.getSealPos().face.getFrontOffsetX() == 0 ? xx / 2 : 0),
	...
	public static AxisAlignedBB getBoundsForArea(ISealEntity seal) {
		return new AxisAlignedBB(seal.getSealPos().pos.getX(), seal.getSealPos().pos.getY(),
				seal.getSealPos().pos.getZ(), seal.getSealPos().pos.getX() + 1, seal.getSealPos().pos.getY() + 1,
				seal.getSealPos().pos.getZ() + 1)
						.offset(seal.getSealPos().face.getFrontOffsetX(), seal.getSealPos().face.getFrontOffsetY(),
								seal.getSealPos().face.getFrontOffsetZ())
						.addCoord(
								seal.getSealPos().face.getFrontOffsetX() != 0
										? (seal.getArea().getX() - 1) * seal.getSealPos().face.getFrontOffsetX()
										: 0,
								seal.getSealPos().face.getFrontOffsetY() != 0
										? (seal.getArea().getY() - 1) * seal.getSealPos().face.getFrontOffsetY()
										: 0,
								seal.getSealPos().face.getFrontOffsetZ() != 0
										? (seal.getArea().getZ() - 1) * seal.getSealPos().face.getFrontOffsetZ()
										: 0)
						.expand(seal.getSealPos().face.getFrontOffsetX() == 0 ? seal.getArea().getX() - 1 : 0,
