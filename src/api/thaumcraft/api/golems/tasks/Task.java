...
public class Task {
	...
	public Task(SealPos sealPos, BlockPos pos) {
		...
		if (sealPos == null) {
			...
			this.id = (System.currentTimeMillis() + "/B/" + sealPos.face.toString() + "/" + sealPos.pos.toString() + "/"
					+ pos.toString()).hashCode();
		this.type = 0;
		this.lifespan = 300;
	}
	...
	public Task(SealPos sealPos, Entity entity) {
		...
		if (sealPos == null) {
			...
			this.id = (System.currentTimeMillis() + "/E/" + sealPos.face.toString() + "/" + sealPos.pos.toString() + "/"
					+ entity.getEntityId()).hashCode();
		this.type = 1;
		this.lifespan = 300;
	}
	...
	// task.golemUUID = new UUID(nbt.getLong("GUUIDMost"),
	// nbt.getLong("GUUIDLeast"));
	//
	// if (nbt.hasKey("EUUIDMost", 4) && nbt.hasKey("EUUIDLeast", 4))
	// task.entityUUID = new UUID(nbt.getLong("EUUIDMost"),
	// nbt.getLong("EUUIDLeast"));
	//
	...
	// SealPos sp = new SealPos(BlockPos.fromLong(nbt.getLong("pos")),
	...
	// nbt.setLong("EUUIDLeast",
	// task.entity.getUniqueID().getLeastSignificantBits());
	// }
