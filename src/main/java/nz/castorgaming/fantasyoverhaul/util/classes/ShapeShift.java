...
public class ShapeShift {
	...
	public ShapeShift() {
		boostWolfman = new StatBoost[] { new StatBoost(0.0f, 0.0, 0.0, 0, 0.0f, 0.0f, 0, 4.0f), new StatBoost(0.0f, 0.0, 0.0, 0, 0.0f, 0.0f, 0, 4.0f),
				new StatBoost(0.0f, 0.0, 0.0, 0, 0.0f, 0.0f, 0, 3.0f), new StatBoost(0.0f, 0.0, 0.0, 0, 0.0f, 0.0f, 0, 3.0f), new StatBoost(0.0f, 0.0, 0.0, 0, 0.0f, 0.0f, 0, 3.0f),
				new StatBoost(0.2f, 0.20000000298023224, 0.20000000298023224, 20, 4.0f, 3.0f, 3, 2.0f), new StatBoost(0.2f, 0.30000001192092896, 0.20000000298023224, 20, 4.0f, 3.0f, 4, 2.0f),
				new StatBoost(0.4f, 0.4000000059604645, 0.4000000059604645, 20, 5.0f, 4.0f, 5, 2.0f), new StatBoost(0.4f, 0.5, 0.4000000059604645, 30, 6.0f, 4.0f, 6, 2.0f),
				new StatBoost(0.5f, 0.6000000238418579, 0.6000000238418579, 40, 7.0f, 5.0f, 7, 2.0f), new StatBoost(0.5f, 0.6000000238418579, 0.6000000238418579, 40, 7.0f, 5.0f, 7, 2.0f) };
		boostWolf = new StatBoost[] { new StatBoost(0.0f, 0.0, 0.0, 0, 0.0f, 0.0f, 0, 4.0f), new StatBoost(0.5f, 0.20000000298023224, 0.20000000298023224, 0, 1.0f, 0.0f, 2, 4.0f),
				new StatBoost(0.5f, 0.20000000298023224, 0.20000000298023224, 0, 1.0f, 0.0f, 2, 3.0f), new StatBoost(0.75f, 0.20000000298023224, 0.30000001192092896, 0, 2.0f, 0.0f, 2, 3.0f),
				new StatBoost(0.75f, 0.20000000298023224, 0.4000000059604645, 0, 2.0f, 0.0f, 3, 3.0f), new StatBoost(0.75f, 0.20000000298023224, 0.5, 0, 2.0f, 0.0f, 3, 2.0f),
				new StatBoost(1.0f, 0.20000000298023224, 0.6000000238418579, 0, 2.0f, 1.0f, 3, 2.0f), new StatBoost(1.25f, 0.30000001192092896, 0.699999988079071, 4, 2.0f, 1.0f, 4, 2.0f),
				new StatBoost(1.5f, 0.30000001192092896, 0.800000011920929, 8, 3.0f, 2.0f, 4, 2.0f), new StatBoost(1.75f, 0.30000001192092896, 0.8999999761581421, 12, 3.0f, 3.0f, 5, 2.0f),
				new StatBoost(1.75f, 0.30000001192092896, 1.0, 12, 3.0f, 3.0f, 5, 2.0f) };
		boostVampire = new StatBoost[] { new StatBoost(0.0f), new StatBoost(1.0f), new StatBoost(1.0f), new StatBoost(1.0f), new StatBoost(2.0f), new StatBoost(2.0f), new StatBoost(2.0f),
				new StatBoost(3.0f), new StatBoost(3.0f), new StatBoost(3.0f), new StatBoost(3.0f) };
		boostBat = new StatBoost[] { new StatBoost(0.0f), new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true),
				new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true),
				new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true), new StatBoost(-6.0f).setFlying(true) };
	...
	public void initCurrentShift(EntityPlayer player) {
		if (!player.worldObj.isRemote) {
			...
			if (boost != null) {
				applyModifier(SharedMonsterAttributes.MOVEMENT_SPEED, ShapeShift.SPEED_MODIFIER, boost.speed, playerAttributes);
				applyModifier(SharedMonsterAttributes.ATTACK_DAMAGE, ShapeShift.DAMAGE_MODIFIER, boost.damage, playerAttributes);
				applyModifier(SharedMonsterAttributes.MAX_HEALTH, ShapeShift.HEALTH_MODIFIER, boost.health, playerAttributes);
			}
			else {
	...
	public void rendArmor(EntityLivingBase victim, EntityPlayer player) {
		ExtendedPlayer playerEx = IExtendPlayer.get(player);
		if (playerEx.getCreatureType() == TransformCreatures.WOLFMAN && IPlayerWerewolf.get(player).getWerewolfLevel() >= 9) {
				...
				for (int i = 0; i < 2 + player.worldObj.rand.nextInt(playerWere.getWerewolfLevel() - 7); ++i) {
					...
					if (creature != null) {
						...
						if (ShapeShift.fieldExperienceValue == null) {
							ShapeShift.fieldExperienceValue = ReflectionHelper.findField(EntityLiving.class, new String[] { "experienceValue", "experienceValue", "aV" });
						}
			...
			if (player.capabilities.isCreativeMode || lastHowl + TimeUtilities.secsToMillisecs(60) < serverTime) {
				...
				for (EntityLivingBase entity : entities) {
					if (!CreatureUtilities.isWerewolf(entity, true) && !CreatureUtilities.isVampire(entity)) {
						entity.addPotionEffect(new PotionEffect(Potions.PARALYSED, TimeUtilities.secsToTicks(4 + player.worldObj.rand.nextInt(playerWere.getWerewolfLevel() - 6)), 3));
					}
				}
			}
			else {
	...
	private void infectPlayerWere(EntityLivingBase entity, float health) {
		...
		if (health < victim.getMaxHealth() * 0.25 && health > 0.0f && !ItemHunterClothes.isWolfProtectionActive(victim)) {
			if (Config.instance().allowVampireWerewolfHybrids || !vicVamp.isVampire()) {
				if (vicWere.getWerewolfLevel() == 0) {
					vicWere.setWerewolfLevel(1);
					ChatUtilities.sendTranslated(TextFormatting.DARK_PURPLE, victim, Reference.WERE_INFECTION, new Object[0]);
				}
			}
		}
	}
	...
	private void infectVillagerWere(EntityLivingBase entity, float health) {
		if (health < entity.getMaxHealth() * 0.25f && health > 0.0f && entity.worldObj.rand.nextInt(4) == 1) {
			...
			EntityWolfman.convertToVillager(villager, villager.getProfessionForge(), false, villager.getHealth(), villager.buyingList);
		}
	}
	...
	public void applyModifier(IAttribute attribute, AttributeModifier modifier, double mod, AbstractAttributeMap playerAttributes) {
		IAttributeInstance attributeInstance = playerAttributes.getAttributeInstance(attribute);
		AttributeModifier speedModifier = new AttributeModifier(modifier.getID(), modifier.getName(), mod, modifier.getOperation());
	...
	static {
		...
		SPEED_MODIFIER = new AttributeModifier(UUID.fromString("10536417-7AA6-4033-A598-8E934CA77D98"), "witcheryWolfSpeed", 0.5, 2);
		DAMAGE_MODIFIER = new AttributeModifier(UUID.fromString("46C5271C-193B-4D41-9CAB-D071AAEE9D4A"), "witcheryWolfDamage", 6.0, 2);
		HEALTH_MODIFIER = new AttributeModifier(UUID.fromString("615920F9-6675-4779-8B18-6A62A3671E94"), "witcheryWolfHealth", 40.0, 0);
	}
