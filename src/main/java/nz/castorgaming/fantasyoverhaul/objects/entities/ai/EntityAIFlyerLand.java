...
public class EntityAIFlyerLand extends EntityAIBase{
    ...
    private boolean liquidBelow(final int y) {
        return this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.living.posX), y, MathHelper.floor_double(this.living.posZ))).getMaterial().isLiquid();
    }
    
    public boolean continueExecuting() {
        final boolean cont = !this.isLanded() && !this.liquidBelow((int)this.living.posY - 1) && !this.liquidBelow((int)this.living.posY);
        return cont;
    }
        ...
        for (int y = y0; y > 1; --y) {
            ...
            if (material != Material.AIR) {
                ...
                for (int i = 0; i < 10; ++i) {
                    ...
                    final int k = MathHelper.floor_double(this.living.getEntityBoundingBox().minY + this.worldObj.rand.nextInt(6) - 3.0);
                    ...
                    double d4 = d0 * d0 + d2 * d2 + d3 * d3;
                    d4 = MathHelper.sqrt_double(d4);
                    if ((blockID.getMaterial() == Material.LEAVES || blockID.getMaterial().isSolid()) && this.worldObj.isAirBlock(new BlockPos(j, k + 1, l)) && this.isCourseTraversable(j, k, l, d4)) {
    ...
    private boolean isLanded() {
        final IBlockState blockID = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.living.posX), (int)(this.living.posY - 0.01), MathHelper.floor_double(this.living.posZ)));
