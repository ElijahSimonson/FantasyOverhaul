...
public class EntityFlyingTameable extends EntityTameable {
	...
	public void moveEntityWithHeading(final float par1, final float par2) {
		if (this.isInWater()) {
			...
			if (this.onGround) {
				f2 = 0.54600006f;
				final Block i = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.posX),
						MathHelper.floor_double(getCollisionBoundingBox().minY) - 1,
						MathHelper.floor_double(this.posZ))).getBlock();
				if (i != Blocks.AIR) {
			...
			if (this.onGround) {
				f2 = 0.54600006f;
				final Block j = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.posX),
						MathHelper.floor_double(getCollisionBoundingBox().minY) - 1,
						MathHelper.floor_double(this.posZ))).getBlock();
				if (j != Blocks.AIR) {
