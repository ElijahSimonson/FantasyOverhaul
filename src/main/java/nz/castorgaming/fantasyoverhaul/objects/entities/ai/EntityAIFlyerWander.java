...
public class EntityAIFlyerWander extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		return (!isTame
				&& this.living.worldObj.getClosestPlayerToEntity((Entity) this.living, this.fleeDistance) != null)
				|| (this.living.getAge() < 100
						&& this.living.getRNG().nextInt(this.living.worldObj.provider.isDaytime() ? 300 : 100) == 0
						&& (!(this.living instanceof EntityTameable) || !((EntityTameable) this.living).isSitting()));
	}

	public boolean continueExecuting() {
		return (this.living instanceof EntityTameable && !((EntityTameable) this.living).isSitting())
				|| this.living.getRNG().nextInt(40) != 0;
	}
	...
	public void updateTask() {
		...
		if (d4 < 1.0 || d4 > 3600.0) {
			final float distance = (this.living instanceof EntityTameable && ((EntityTameable) this.living).isTamed())
					? 2.0f
					: 6.0f;
