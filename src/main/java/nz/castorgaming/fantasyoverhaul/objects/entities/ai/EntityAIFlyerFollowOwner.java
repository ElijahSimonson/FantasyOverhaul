...
public class EntityAIFlyerFollowOwner extends EntityAIBase{
    ...
    public EntityAIFlyerFollowOwner(final EntityTameable par1EntityTameable, final double par2, final float par4, final float par5) {
    ...
    public boolean shouldExecute() {
        ...
        if (this.thePet.dimension != entitylivingbase.dimension || this.thePet.getDistanceSqToEntity((Entity)entitylivingbase) < this.minDist * this.minDist) {
            return false;
        }
    ...
    public boolean continueExecuting() {
        return this.thePet.getDistanceSqToEntity((Entity)this.theOwner) > this.maxDist * this.maxDist && !this.thePet.isSitting();
    }
        ...
        if (!this.thePet.isSitting() && --this.field_75343_h <= 0) {
            ...
            if (this.thePet.dimension != this.theOwner.dimension || this.thePet.getDistanceSqToEntity((Entity)this.theOwner) >= 256.0) {
                ...
                for (int dx = 0; dx <= 4; ++dx) {
                    for (int dz = 0; dz <= 4; ++dz) {
                        for (int dy = 0; dy <= 4; ++dy) {
                            ...
                            final int newY = y + dy;
                            final int newZ = z + dz;
                            if (this.theOwner.worldObj.getBlockState(new BlockPos(newX, newY - 1, newZ)).isSideSolid((IBlockAccess)this.theOwner.worldObj, new BlockPos(newX, newY - 1, newZ), EnumFacing.UP) && !this.theOwner.worldObj.getBlockState(new BlockPos(newX, newY, newZ)).isNormalCube() && !this.theOwner.worldObj.getBlockState(new BlockPos(newX, newY + 1, newZ)).isNormalCube()) {
                                EntityUtil.teleportToLocation(this.theWorld, 0.5 + newX, 0.01 + newY, 0.5 + newZ, this.theOwner.dimension, (Entity)this.thePet, true);
                                return;
                            }
                        }
                    }
                ...
                if (this.isCourseTraversable(this.theOwner.posX, this.theOwner.posY, this.theOwner.posZ, d4)) {
                    ...
                    final double newX2 = this.thePet.posX + (this.thePet.worldObj.rand.nextFloat() * 8.0f - 4.0f) * 6.0f;
                    final double newY2 = this.thePet.posY + (this.thePet.worldObj.rand.nextFloat() * 2.0f - 1.0f) * 6.0f;
                    final double newZ2 = this.thePet.posZ + (this.thePet.worldObj.rand.nextFloat() * 8.0f - 4.0f) * 6.0f;
