...
public class ItemBatBall extends GeneralItem {
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			EntityItem item = new EntityItem(worldIn, playerIn.posX, playerIn.posY + 1.3, playerIn.posZ,
					ItemInit.BAT_BALL.createStack());
			item.setPickupDelay(5);
			item.setLocationAndAngles(playerIn.posX, playerIn.posY + playerIn.getEyeHeight(), playerIn.posZ,
			...
			float f = 0.4f;
			item.motionX = -MathHelper.sin((float) (item.rotationYaw / 180.0f * Math.PI))
					* MathHelper.cos((float) (item.rotationPitch / 180.0f * Math.PI)) * f;
			item.motionZ = MathHelper.cos((float) (item.rotationYaw / 180.0f * Math.PI))
					* MathHelper.cos((float) (item.rotationPitch / 180.0f * Math.PI)) * f;
