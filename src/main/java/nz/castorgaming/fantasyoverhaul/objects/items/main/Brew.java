...
public class Brew extends Drinkable{
	...
	public BrewResult onImpact(World world, EntityLivingBase thrower, RayTraceResult rtr, boolean enhanced, double x, double y, double z, AxisAlignedBB bounds) {
	...
	protected static boolean setBlockIfNotSolid(World world, BlockPos pos, Block block, IBlockState state) {
		if (!world.getBlockState(pos).getMaterial().isSolid() || (block == Blocks.WEB && BlockUtil.getBlock(world, pos) == Blocks.SNOW)) {
			BlockUtil.setBlock(world, pos, state);
			ParticleEffect.EXPLODE.send(SoundEffect.NONE, world, 0.5 + pos.getX(), 0.5 + pos.getY(), 0.5 + pos.getZ(), 1.0, 1.0, 16);
			return true;
		}
		return false;
	}
	...
	public void throwBrew(ItemStack stack, World world, EntityPlayer player) {
		...
		world.playSound(player, player.getPosition(), SoundEffect.RANDOM_BOW.event(), SoundCategory.PLAYERS, 0.5f, 0.4f / (GeneralItem.itemRand.nextFloat() * 0.4f + 0.8f));
		if (!world.isRemote) {
