...
public class InfernalBlood extends GeneralItem{
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos,
		...
		if (state.getBlock() == BlockInit.WICKER_BUNDLE && BlockWickerBundle.limitToValidState(state) == BlockWickerBundle.Type.DRY) {
			if (!worldIn.isRemote) {
				for (int uses = 5, y = pos.getY() - 1; y <= pos.getY() + 1 && uses > 0; y++) {
					for (int x = pos.getX() - 1; x <= pos.getX() + 1 && uses > 0; x++) {
						for (int z = pos.getZ() - 1; z <= pos.getZ() + 1 && uses > 0; z++) {
							...
							if (blockState.getBlock() == BlockInit.WICKER_BUNDLE && BlockWickerBundle.limitToValidState(state) == BlockWickerBundle.Type.DRY) {
								worldIn.setBlockState(currPos, BlockInit.WICKER_BUNDLE.getDefaultState().withProperty(BlockWickerBundle.STAINED, true));
								--uses;
							}
						}
					}
