...
public class GeneralItem extends ItemBase {
	...
	public String getBoundDisplayName(ItemStack stack) {
		...
		if (tag != null && tag.hasKey("PosX") && tag.hasKey("PosY") && tag.hasKey("PosZ") && tag.hasKey("NameD")) {
			return String.format("%s: %d, %d, %d", tag.getString("NameD"), tag.getInteger("PosX"),
					tag.getInteger("PosY"), tag.getInteger("PosX"));
		}
		return "";
	}

	public void bindToLocation(World world, int posX, int posY, int posZ, int dimension, String dimensionName,
			ItemStack stack) {
	...
	public boolean hasEffect(ItemStack stack, int pass) {
		return (pass == 0) && stack.isItemEnchanted() && ItemInit.BROOM_ENCHANTED.isMatch(stack)
				|| ItemInit.SPIRIT_SUBDUED.isMatch(stack);
	}
	...
	public int getMaxItemUseDuration(ItemStack stack) {
		...
		if (item instanceof Edible || item instanceof Drinkable) {
			return 32;
		} else if (ItemInit.WAYSTONE_BOUND.isMatch(stack) || ItemInit.CONTRACT_TORMENT.isMatch(stack)
				|| ItemInit.SEER_STONE.isMatch(stack)) {
			return 1200;
		}
	...
	protected boolean circleNear(World world, EntityPlayer player) {
		...
		int[][] PATTERN = { { 0, 0, 0, 0, 4, 3, 4, 0, 0, 0, 0 }, { 0, 0, 4, 3, 1, 1, 1, 3, 4, 0, 0 },
				{ 0, 4, 1, 1, 1, 1, 1, 1, 1, 4, 0 }, { 0, 3, 1, 1, 1, 1, 1, 1, 1, 3, 0 },
				{ 4, 1, 1, 1, 2, 2, 2, 1, 1, 1, 4 }, { 3, 1, 1, 1, 2, 1, 2, 1, 1, 1, 3 },
				{ 4, 1, 1, 1, 2, 2, 2, 1, 1, 1, 4 }, { 0, 3, 1, 1, 1, 1, 1, 1, 1, 3, 0 },
				{ 0, 4, 1, 1, 1, 1, 1, 1, 1, 4, 0 }, { 0, 0, 4, 3, 1, 1, 1, 3, 4, 0, 0 },
		...
		for (int z = 0; z < PATTERN.length - 1; z++) {
			...
			for (int x = 0; x < PATTERN[z].length; x++) {
				...
				if (value != 0 && isPost(world, worldX, midY, worldZ, value == 2 || value == 4, value == 4,
						value == 3 || value == 4)) {
					return false;
				}
			}
	...
	private boolean placeBlock(Block spawnBlock, ItemStack stack, EntityPlayer player, World world, BlockPos pos,
		...
		if (world.checkBlockCollision(bounds)) {
			IBlockState state = spawnBlock.onBlockPlaced(world, pos, facing, hitX, hitY, hitZ,
					spawnBlock.getMetaFromState(spawnBlock.getDefaultState()), player);
			placeCheckedBlock(spawnBlock, stack, player, world, pos, facing, hitX, hitY, hitZ, state);
		}
		return true;
	}

	private void placeCheckedBlock(Block spawnBlock, ItemStack stack, EntityPlayer player, World world, BlockPos pos,
			EnumFacing facing, float hitX, float hitY, float hitZ, IBlockState state) {
	...
	public void setHeadingFromThrower(EntityItem item, Entity entityThrower, float rotationPitchIn, float rotationYawIn,
