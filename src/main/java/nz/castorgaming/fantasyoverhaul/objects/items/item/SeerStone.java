...
public class SeerStone extends GeneralItemEnchanted {
	...
	private void readPlayer(EntityPlayer player, EntityPlayer target) {
		...
		if (nbtTarget != null && nbtTarget.hasKey(Reference.MANIFESTATION_COUNTDOWN)) {
			...
			if (timeRemaining > 0) {
				read += String.format(GeneralUtil.resource("item.seerstone.manifestationtime"),
						Integer.valueOf(timeRemaining).toString()) + ", ";
			} else {
		...
		if (covenSize > 0) {
			read += String.format(GeneralUtil.resource("item.seerstone.covensize"),
					Integer.valueOf(covenSize).toString()) + ", ";
		} else {
		...
		if (extPlayer != null) {
			...
			read += String.format(GeneralUtil.resource("item.seerstone.bottlingskill"),
					Integer.valueOf(bottlingSkill).toString()) + ", ";
		}
		if (nbtTarget != null && (nbtTarget.hasKey(Reference.INFUSION_CURSED)
				|| nbtTarget.hasKey(Reference.INFUSION_INSANITY) || nbtTarget.hasKey(Reference.INFUSION_SINKING)
				|| nbtTarget.hasKey(Reference.INFUSION_OVERHEAT) || nbtTarget.hasKey(Reference.WAKING_NIGHTMARE))) {
