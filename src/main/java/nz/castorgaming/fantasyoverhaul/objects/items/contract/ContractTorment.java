		...
		if (!world.isRemote && player instanceof EntityPlayerMP) {
			if (elapsedTicks == 0 || elapsedTicks == 40) {
				...
				if (circleNear(world, player)) {
					...
					ParticleEffect.FLAME.send(SoundEffect.NONE, player, 1.0, 2.0, 16);
					player.resetActiveHand();
					EntityLiving living = Infusion.spawnCreature(world, EntityLordOfTorment.class, player.posX,
							player.posY, player.posZ, null, 2, 4, ParticleEffect.FLAME,
							SoundEffect.MOB_ENDERDRAGON_GROWL);
					if (living != null) {
						...
						world.newExplosion(living, living.posX, living.posY, living.posZ, 7.0f, false,
