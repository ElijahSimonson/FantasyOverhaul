...
public class ItemMutandis extends GeneralItem{
		...
		if (!worldIn.isRemote) {
			...
			if (isExtreme && (block == Blocks.GRASS || block == Blocks.MYCELIUM)) {
				if (worldIn.rand.nextInt(2) == 0) {
					worldIn.setBlockState(pos, (block == Blocks.GRASS) ? Blocks.MYCELIUM.getDefaultState() : Blocks.GRASS.getDefaultState());
					ParticleEffect.INSTANT_SPELL.send(SoundEffect.RANDOM_FIZZ, worldIn, new BlockPos(pos).up(), 1.0, 1.0, 16);
					--stack.stackSize;
				}
			}
			else if (isExtreme && block == Blocks.DIRT && (blockAbove == Blocks.WATER || blockAbove == Blocks.FLOWING_WATER)) {
				if (worldIn.rand.nextInt(2) == 0) {
				...
				MutableBlocks [] blocks = {new MutableBlock(Blocks.SAPLING, 0), new MutableBlock(Blocks.SAPLING, 1), new MutableBlock(Blocks.SAPLING, 2), new MutableBlock(Blocks.SAPLING, 3), new MutableBlock(Blocks.SAPLING, 4), new MutableBlock(Blocks.SAPLING, 5), new MutableBlock(BlockInit.SAPLING, 0), new MutableBlock(BlockInit.SAPLING, 1), new MutableBlock(BlockInit.SAPLING, 2), new MutableBlock(BlockInit.EMBER_MOSS, 0), new MutableBlock((Block)Blocks.TALLGRASS, 1), new MutableBlock(Blocks.WATERLILY), new MutableBlock((Block)Blocks.BROWN_MUSHROOM_BLOCK), new MutableBlock((Block)Blocks.RED_MUSHROOM_BLOCK), new MutableBlock((Block)Blocks.RED_FLOWER, 0), new MutableBlock((Block)Blocks.YELLOW_FLOWER), new MutableBlock(BlockInit.SPANISH_MOSS, 1) };
				...
				if (isExtreme) {
					MutableBlock[] extremisBlocks = { new MutableBlock(Blocks.CARROTS, -1, Math.min(metadata, 7)), new MutableBlock(Blocks.POTATOES, -1, Math.min(metadata, 7)), new MutableBlock(Blocks.WHEAT, -1, Math.min(metadata, 7)), new MutableBlock(Blocks.REEDS, -1, Math.min(metadata, 7)), new MutableBlock(BlockInit.CROP_BELLADONNA, -1, Math.min(metadata, BlockInit.CROP_BELLADONNA.getNumGrowthStages())), new MutableBlock(BlockInit.CROP_MANDRAKE, -1, Math.min(metadata, BlockInit.CROP_MANDRAKE.getNumGrowthStages())), new MutableBlock(BlockInit.CROP_ARTICHOKE, -1, Math.min(metadata, BlockInit.CROP_ARTICHOKE.getNumGrowthStages())), new MutableBlock(Blocks.PUMPKIN_STEM, -1, Math.min(metadata, 7)), new MutableBlock(Blocks.CACTUS), new MutableBlock(Blocks.MELON_STEM, -1, Math.min(metadata, 7)), new MutableBlock(Blocks.NETHER_WART, -1, Math.min(metadata, 3)) };
	...
	public static void setBlockToClay(World world, BlockPos pos) {
		...
		if (block == Blocks.DIRT && (blockAbove == Blocks.WATER || blockAbove == Blocks.FLOWING_WATER)) {
			...
			if (!world.isRemote) {
				ParticleEffect.INSTANT_SPELL.send(SoundEffect.MOB_SLIME_BIG, world, pos.add(0.5, 1.5, 0.5), 1.0, 1.0, 16);
			}
		}
	}
}
