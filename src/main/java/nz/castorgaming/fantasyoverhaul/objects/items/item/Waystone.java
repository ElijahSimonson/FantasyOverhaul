...
public class Waystone extends GeneralItem{
	...
	private boolean isRestrictedTeleportTarget(int source, int target) {
		return source != target && (source == Config.instance().dimensionDreamID || source == Config.instance().dimensionMirrorID || target == Config.instance().dimensionDreamID || target == Config.instance().dimensionMirrorID);
	}
	...
	public boolean teleportToLocation(World world, ItemStack stack, Entity entity, int radius, boolean presetPosition) {
		...
		if (tag != null && tag.hasKey("PosX") && tag.hasKey("PosY") && tag.hasKey("PosZ") && tag.hasKey("PosD")) {
			...
			if (entity != null && target != null && !isRestrictedTeleportTarget(entity.dimension, target.dimension)) {
				TeleportUtil.teleportToLocation(world, MathHelper.floor_double(target.posX), MathHelper.floor_double(target.posY), MathHelper.floor_double(target.posZ), target.dimension, entity, presetPosition);
				return true;
			}
		}
		return false;
