...
public class InfusionOtherwhere extends Infusion {
	...
	@Override
	public void onLeftClickEntity(final ItemStack itemstack, final World world, final EntityPlayer player, final Entity otherEntity) {
		if (world.isRemote) {
			return;
		}
		if (otherEntity instanceof EntityLivingBase) {
			...
			if (player.isSneaking()) {
				...
				if (recallLocation != null && recallLocation.dimension != Config.instance().dimensionDreamID && recallLocation.dimension != Config.instance().dimensionTormentID
						&& recallLocation.dimension != Config.instance().dimensionMirrorID && world.provider.getDimension() != Config.instance().dimensionDreamID
						&& world.provider.getDimension() != Config.instance().dimensionTormentID && world.provider.getDimension() != Config.instance().dimensionMirrorID
					...
					if (player instanceof EntityPlayerMP && !isConnectionClosed((EntityPlayerMP) player)) {
						...
						EntityUtil.teleportToLocation(world, recallLocation.posX, recallLocation.posY, recallLocation.posZ, recallLocation.dimension, player, true);
						otherLivingEntity.fallDistance = 0.0f;
						if (!EnderInhibition.isActive(otherLivingEntity, 2)) {
							EntityUtil.teleportToLocation(world, recallLocation.posX, recallLocation.posY, recallLocation.posZ, recallLocation.dimension, otherLivingEntity, true);
						}
					}
				}
				else {
					world.playSound(player, player.getPosition(), SoundEffect.NOTE_PLING.event(), SoundEffect.NOTE_PLING.category(), 0.5f, 0.4f / ((float) Math.random() * 0.4f + 0.8f));
				}
			}
				...
				if (player instanceof EntityPlayerMP && !isConnectionClosed((EntityPlayerMP) player) && hikeModified > 0.0 && hikeModified2 > 0.0) {
					...
					if (!EnderInhibition.isActive(otherLivingEntity, 2)) {
						EntityUtil.teleportToLocation(world, otherLivingEntity.posX, otherLivingEntity.posY + hikeModified2, otherLivingEntity.posZ, otherLivingEntity.dimension, otherLivingEntity,
								true);
					}
				}
			}
	...
	@Override
	public void onUsingItemTick(final ItemStack itemstack, final World world, final EntityPlayer player, final int countdown) {
		final int elapsedTicks = this.getMaxItemUseDuration(itemstack) - countdown;
		if (player.isSneaking() && elapsedTicks == 60) {
			if (!world.isRemote) {
				ChatUtilities.sendTranslated(TextFormatting.GRAY, player, "witchery.infuse.cansetrecall", new Object[0]);
			}
			player.worldObj.playSound(player, player.getPosition(), SoundEvents.BLOCK_NOTE_PLING, SoundCategory.PLAYERS, 0.5f, 0.4f / ((float) Math.random() * 0.4f + 0.8f));
		}
			...
			if (hitMOP != null) {
				player.worldObj.playSound(player, player.getPosition(), SoundEffect.RANDOM_ORB.event(), SoundEffect.RANDOM_ORB.category(), 0.5f, 0.4f / ((float) Math.random() * 0.4f + 0.8f));
				if (!world.isRemote) {
					ChatUtilities.sendTranslated(TextFormatting.GRAY, (ICommandSender) player, "witchery.infuse.canteleport", new Object[0]);
				}
			}
			else {
				player.worldObj.playSound(player, player.getPosition(), SoundEffect.RANDOM_POP.event(), SoundEffect.RANDOM_POP.category(), 0.5f, 0.4f / ((float) Math.random() * 0.4f + 0.8f));
			}
		}
	}

	@Override
	public void onPlayerStoppedUsing(final ItemStack itemstack, final World world, final EntityPlayer player, final int countdown) {
		if (world.isRemote) {
			return;
		}
		final int elapsedTicks = this.getMaxItemUseDuration(itemstack) - countdown;
		if (player.isSneaking() && elapsedTicks >= 60) {
			...
			if (recallLocation != null && recallLocation.dimension != Config.instance().dimensionDreamID && recallLocation.dimension != Config.instance().dimensionTormentID
				...
				world.playSound(player, player.getPosition(), SoundEffect.NOTE_SNARE.event(), SoundEffect.NOTE_SNARE.category(), 0.5f, 0.4f / ((float) Math.random() * 0.4f + 0.8f));
			}
		}
		else {
			final int MAX_TELEPORT_DISTANCE = 40 + 20 * (elapsedTicks / 20);
			final RayTraceResult hitMOP = doCustomRayTrace(world, player, true, MAX_TELEPORT_DISTANCE);
			if (hitMOP != null && !EnderInhibition.isActive(player, 2) && this.consumeCharges(world, player, 1, false)) {
				...
				if (hitMOP == null && !world.isRemote) {
					ChatUtilities.sendTranslated(TextFormatting.RED, (ICommandSender) player, "witchery.infuse.cannotteleport", new Object[0]);
				}
			}
		}
	}
	...
	private void storeLocation(final NBTTagCompound nbt, final String key, final EntityPlayer player) {
		...
		if (!player.worldObj.isRemote) {
			ChatUtilities.sendTranslated(TextFormatting.GRAY, player, "witchery.infuse.setrecall", player.worldObj.provider.getDimension(),
					Integer.valueOf(MathHelper.floor_double(location.posX)).toString(), Integer.valueOf(MathHelper.floor_double(location.posY)).toString(),
	...
	public static RayTraceResult doCustomRayTrace(final World world, final EntityPlayer player, final boolean collisionFlag, final double reachDistance) {
	...
	public static RayTraceResult raytraceEntities(final World world, final EntityPlayer player, final boolean collisionFlag, final double reachDistance) {
		RayTraceResult pickedEntity = null;
		final Vec3d playerPosition = new Vec3d(player.posX, player.posY + player.getEyeHeight(), player.posZ);
		final Vec3d playerLook = player.getLookVec();
		final Vec3d playerViewOffset = new Vec3d(playerPosition.xCoord + playerLook.xCoord * reachDistance, playerPosition.yCoord + playerLook.yCoord * reachDistance,
	...
	public static RayTraceResult raytraceBlocks(final World world, final EntityPlayer player, final boolean collisionFlag, final double reachDistance) {
		final Vec3d playerPosition = new Vec3d(player.posX, player.posY + player.getEyeHeight(), player.posZ);
		final Vec3d playerLook = player.getLookVec();
		final Vec3d playerViewOffset = new Vec3d(playerPosition.xCoord + playerLook.xCoord * reachDistance, playerPosition.yCoord + playerLook.yCoord * reachDistance,
				playerPosition.zCoord + playerLook.zCoord * reachDistance);
	...
	private static RayTraceResult raytraceUpBlocks(final World world, final EntityLivingBase player, final boolean collisionFlag, final double reachDistance) {
		final Vec3d playerPosition = new Vec3d(player.posX, player.posY + player.getEyeHeight(), player.posZ);
		final Vec3d playerUp = new Vec3d(0.0, 1.0, 0.0);
		final Vec3d playerViewOffset = new Vec3d(playerPosition.xCoord + playerUp.xCoord * reachDistance, playerPosition.yCoord + playerUp.yCoord * reachDistance,
